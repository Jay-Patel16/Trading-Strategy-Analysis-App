...
def oneDayData(asset, startDate, EndDate):
    dataOneDay =  yf.download(asset, start = startDate, end = EndDate, interval='1m')
    return dataOneDay
def trendFollowing(data,buyPercent, sellPercent,hour):
    ...
    while hourCount < 6:
        ...
        percentChange = ((startingPrice - dayStartPrice)/ (abs(dayStartPrice)) ) * 100
        percentChangeSell = ((SellPrice - startingPrice)/ (abs(startingPrice)) ) * 100
        i = startTime
        j = startTime+1
        foundSell = 0
        if percentChange >= buyPercent:
            ...
            while i <len(data)-1:
                ...
                percentChangeSell = ((SellPrice - BroughtPrice)/ (abs(BroughtPrice)) ) * 100
...
def rangeTrendFollowing(asset, startDate, endDate,buyP, sellP, startHour):
    ...
    for i in range (1,len(dataRange)-1):
        dataOneDay = oneDayData(asset, dataRange.iloc[i].name, dataRange.iloc[i+1].name)
